...
COLUMN_NAMES = ['T3SS_protein', 'bacterial_protein', "identity_percent", "alignment length", "mismatch", "gapopen", "query_start", "query_end", "query_ length", 'query_coverage_percentage', "target_ start", "target_end", 'e_value', 'bit_score']
...
def get_mmseqs_results_dictionary(mmseqs_results_file):
    ...
    df = pd.read_csv(mmseqs_results_file, sep='\t', names=COLUMN_NAMES, header=None)
...
def get_best_bacterial_T3SS_match_dict(all_subsystems_dict):
    ...
    for homologous_bacterial_gene in T3SS_homologous_bacterial_genes:
        ...
        if best_system_gene is not None and best_subsystem is not None:
            best_bacterial_T3SS_match[homologous_bacterial_gene] = (best_system_gene, best_subsystem)

    return best_bacterial_T3SS_match


def get_full_bacterial_T3SS_dict(T3SS_data, best_bacterial_T3SS_match_dict):
    full_bacterial_T3SS_dict = {} # If a homologous bacterial gene is found - {bacterial_gene: (system_gene, subsystem)}. If no homologous bacterial gene is found - {integer: (system_gene, subsystem)}
    i = 1
...
def main():
    ...
    parser.add_argument('T3SS_data', type=str, help='Path to T3SS datasets directory')
    args = parser.parse_args()

    bacterial_proteome = os.path.join(args.working_directory, args.bacterial_proteome)
